...
import (
	...
	"github.com/50HJ/Intelli-Mall/internal/logger/log"
...
func (s *productCacheSuite) SetupSuite() {
	...
	const dbUrl = "postgres://intellimall_user:intellimall_pass@localhost:%s/intellimall?sslmode=disable"
	...
	s.db, err = sql.Open("pgx", fmt.Sprintf("postgres://intellimall_user:intellimall_pass@%s/intellimall?sslmode=disable", endpoint))
	if err != nil {
		s.T().Fatal(err)
	}
